...
//    self.navigationItem.title = _model.name;
    ...
    titleBTn.frame = CGRectMake(20, 44, 160, 40);
    [titleBTn setTitle:@"全部照片" forState:UIControlStateNormal];
    [titleBTn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [titleBTn setImage:[UIImage imageNamed:@"xiala123"] forState:UIControlStateNormal];
    [titleBTn addTarget:self action:@selector(showAllQuestions) forControlEvents:UIControlEventTouchUpInside];
    CGFloat leftlab = [titleBTn.titleLabel.text sizeWithAttributes:@{NSFontAttributeName : titleBTn.titleLabel.font}].width;
    [titleBTn setImageEdgeInsets:UIEdgeInsetsMake(0, leftlab+20, 0, 0)];
    [titleBTn setTitleEdgeInsets:UIEdgeInsetsMake(0, -30, 0, 0)];
  if (@available(iOS 13.0, *)) {
    [titleBTn setImageEdgeInsets:UIEdgeInsetsMake(0, leftlab+30, 0, 0)];
    [titleBTn setTitleEdgeInsets:UIEdgeInsetsMake(0, -40, 0, 0)];
  }
    ...
    UIBarButtonItem *cancelItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"rightItem"] style:UIBarButtonItemStylePlain target:self action:@selector(cancelButtonClick)];
...
//  }];
    ...
    UIBarButtonItem *nextItem = [[UIBarButtonItem alloc] initWithTitle:@"下一步" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonClick)];
//    [nextItem setTintColor:[UIColor colorWithHexString:@"9d9d9d"]];
...
- (void)viewDidLayoutSubviews {
    ...
    _collectionView.frame = CGRectMake(0, top, self.view.tz_width, self.view.tz_height - top);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        ...
        if ([tzImagePickerVc.takePictureImageName isEqualToString:@"3"]) {
...
//                [self.navigationItem.rightBarButtonItem setTintColor:[UIColor colorWithHexString:@"9d9d9d"]];
                self.navigationItem.rightBarButtonItem.enabled = false;
            }else{
                
                [self.navigationItem.rightBarButtonItem setTintColor:[UIColor colorWithRed:255/255.0f green:78/255.0f blue:104/255.0f alpha:1.0f]];
            ...
            [strongSelf refresAWTottomToolBarStatus];
...
- (void)refresAWTottomToolBarStatus {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
...
- (void)addPHAsset:(PHAsset *)asset {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            ...
            [tzImagePickerVc addSelectedModel:assetModel];
            [self refresAWTottomToolBarStatus];
        }
    }
